...
func (c *Consumer) Start(ctx context.Context, cfg ConsumerConfig) {
	...
	go func() {
		for {
			select {
			case <-ctx.Done():
				log.Printf("🛑 Stopping Consumer [%s] loop due to context cancellation", cfg.ConsumerName)
				return
			default:
				err := c.consumeLoop(ctx, cfg)
				if err != nil {
...
func (c *Consumer) consumeLoop(ctx context.Context, cfg ConsumerConfig) error {
	...
	log.Printf("🚀 Consumer [%s] started (Parallelism: %d) on queue [%s]...", cfg.ConsumerName, cfg.PrefetchCount, cfg.QueueName)
	...
	for {
		select {
		case <-ctx.Done():
			log.Printf("🛑 Consumer [%s] shutting down, waiting for in-flight messages...", cfg.ConsumerName)
			wg.Wait()
			return nil
		case msg, ok := <-msgs:
			if !ok {
...
func (c *Consumer) handleDelivery(msg amqp.Delivery, cfg ConsumerConfig) {
	...
	if err != nil {
		...
		if retryCount >= cfg.MaxRetries {
			log.Printf("❌ Consumer [%s]: Max retry reached (%d), sending to final DLQ", cfg.ConsumerName, retryCount)
		...
		log.Printf("🔁 Consumer [%s]: Processing failed, triggering delayed retry via Nack: %d/%d", cfg.ConsumerName, retryCount+1, cfg.MaxRetries)
